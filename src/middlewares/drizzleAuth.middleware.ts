...
export const drizzleAuthMiddleware = (allowedRoles: String[]) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if(decoded.role === "customer") {
        user = await customerRepository.findById(decoded.id)
      } else if (decoded.role === "serviceProvider") {
        user = await serviceProviderRepository.findById(decoded.id)
